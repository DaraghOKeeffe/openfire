...
public class DbRuleManager {
    ...
    private static final String GET_LAST_RULEID =
        "SELECT id from ofPfRules order by id DESC";

    private static final String GET_FIRST_ORDERID =
    	"SELECT ruleorder from ofPfRules order by ruleorder ASC";
    ...
    public int getFirstOrderId() {
        ...
        try {
            con = DbConnectionManager.getConnection();
            pstmt = con.prepareStatement(GET_FIRST_ORDERID);
            rs = pstmt.executeQuery();
            if (rs.next()) {
    ...
    /**
     * Returns the id of the last rule added.
     * @return
     */
    public String getLastRuleId() {
        ...
        try {
            con = DbConnectionManager.getConnection();
            pstmt = con.prepareStatement(GET_LAST_RULEID);
            rs = pstmt.executeQuery();
            if (rs.next()) {
    ...
    public boolean addRule(Rule rule, Integer order) {
        ...
        try {
            ...
            pstmt.execute();
            // set the auto generated id of this rule
            // not an ideal solution, another rule could be
            // added inbetween. Temp solution as not all the db types
            // supported by Openfire support the RETURNING clause
            // of insert
            rule.setRuleId(this.getLastRuleId());
            rules.clear();
...
//See if there is a gap that we can just update the rule with
        ...
        //No gap. Move all rules >= to the order.
            ...
            for (Rule moveRule: rules) {
                
                if (new Integer(moveRule.getOrder()) >= order) {
                    updateRule(moveRule, moveRule.getOrder() + 1);
                } else break;
            }
            updateRule(rule, order);
        }
    ...
    public boolean updateRule(Rule rule, Integer order) {
        ...
        try {
            ...
            Log.info("Rule null?: "+(rule==null) );
            Log.info("Rule Packet Type: "+rule.getPackeType().toString());
